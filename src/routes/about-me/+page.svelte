<div class="grid grid-cols-12 space-x-4" style="min-height: calc(100vh - 128px);">
  <div class="md:col-span-4 sm:col-span-12 flex justify-center items-center">
    <img class="rounded-2xl w-full m-4" src="https://placehold.co/200x250" alt="Profile" />
  </div>
  <div class="md:col-span-8 sm:col-span-12">
    <div class="flex items-center h-full">
      <div>
        <h1 class="text-3xl font-bold mb-4 font-rubik">About Me</h1>
        <p class="text-lg mb-4 font-rubik">
          Hello, my name is Lukas Kohlmaier. I am a full-stack-developer and
          keyboard enthusiast currently working in Klagenfurt, Austria. I am
          passionate about programming and love to learn new things.
        </p>
        <p class="text-lg mb-4 font-rubik">
          I am
          working with the following technologies:
        </p>

        <h2 class="font-rubik">Backend</h2>
        <ul class="list-disc list-inside font-rubik">
          <li class="ml-6">asp.net core</li>
          <li class="ml-6">graphql (currently learning)</li>
          <li class="ml-6">ef core</li>
          <li class="ml-6">microsoft sql server / postgres</li>
        </ul>

        <h2 class="font-rubik">Frontend</h2>
        <ul class="list-disc list-inside font-rubik">
          <li class="ml-6">React</li>
          <li class="ml-6">next.js</li>
          <li class="ml-6">svelte (currently learning)</li>
          <li class="ml-6">ant design</li>
          <li class="ml-6">tailwind</li>
          <li class="ml-6">material ui</li>
        </ul>

        <h2 class="font-rubik">Cloud</h2>
        <ul class="list-disc list-inside font-rubik">
          <li class="ml-6">Microsoft Azure</li>
        </ul>

        <p class="text-lg mb-4 mt-4 font-rubik">
          Do you have a project in mind? Feel free to reach out via the contact
          form. I'm sure we can build something great together.
        </p>
      </div>
    </div>
  </div>
</div>
